            ...
            <div :style="{ display: 'flex' }">
                <div style="flex: 1;overflow: auto;">
                    <a-form ref="formRef" :model="form.data" :rules="(form.rules as any)" layout="vertical"
                        ...
                        <a-form-item field="title.zh-CN" :label="$t('match.update.5ukj6414rvw0')">
                            <a-input v-model="form.data.title['zh-CN']" :placeholder="$t('match.update.5ukj6414si80')" />
                        </a-form-item>
                        <a-form-item field="title.en" :label="$t('match.update.5ukj6414smk0')">
                            <a-input v-model="form.data.title.en" :placeholder="$t('match.update.5ukj6414sow0')" />
                        </a-form-item>
                        <a-form-item field="title.tc" :label="$t('match.update.5ukj6414srg0')">
                            <a-input v-model="form.data.title.tc" :placeholder="$t('match.update.5ukj6414sts0')" />
                        </a-form-item>
                        <a-form-item field="type" :label="$t('match.update.5ukj6414sxs0')">
                            <a-select v-model="form.data.type" :placeholder="$t('match.update.5ukj6414t000')">
                                <a-option v-for="item in useEnums('market.market_type')" :value="item.value">{{
                                    item.trans[local.lang] }}</a-option>
                            </a-select>
                        </a-form-item>
                        <a-form-item field="period" :label="$t('match.update.5ukj6414t2c0')">
                            <a-select v-model="form.data.period" :placeholder="$t('match.update.5ukj6414t000')">
                                <a-option v-for="item in useEnums('cms.operate.symbol.match.period')" :value="item.value">{{
                                    item.trans[local.lang] }}</a-option>
                            </a-select>
                        </a-form-item>
                        <a-form-item field="is_cycle" :label="$t('match.update.5ukj6414t7w0')">
                            <a-select v-model="form.data.is_cycle" :placeholder="$t('match.update.5ukj6414t000')">
                                <a-option v-for="item in useEnums('cms.operate.quote.market.status')" :value="item.value">{{
                                    item.trans[local.lang] }}</a-option>
                            </a-select>
                        </a-form-item>
                        <a-form-item field="start_time" :label="$t('match.update.5ukj6414tac0')">
                            <a-date-picker value-format="x" style="width:100%" show-time v-model="form.data.start_time"
                                :placeholder="$t('match.update.5ukj6414tac0')" />
                        </a-form-item>
                        <a-form-item field="end_time" :label="$t('match.update.5ukj6414tcs0')">
                            <a-date-picker value-format="x" style="width:100%" show-time v-model="form.data.end_time"
                                :placeholder="$t('match.update.5ukj6414tcs0')" />
                        </a-form-item>
                        <a-form-item field="banner" :label="$t('match.update.5ukj6414tf40')">
                            <div style="width: 100%">
                                ...
                                <p style="margin-top:15px">{{$t('match.update.5ukj6414ths0')}}</p>
                            </div>
                        </a-form-item>
                        <a-form-item field="background" :label="$t('match.update.5ukj6414tk00')">
                            <div style="width: 100%">
                                ...
                                <p style="margin-top:15px">{{$t('match.update.5ukj6414tnk0')}}</p>
                            </div>
                        </a-form-item>
                        <a-form-item field="status" :label="$t('match.update.5ukj6414tq00')">
                            <a-select v-model="form.data.status" :placeholder="$t('match.update.5ukj6414t000')">
                                <a-option v-for="item in useEnums('cms.operate.quote.market.status')" :value="item.value">{{
                                    item.trans[local.lang] }}</a-option>
                            </a-select>
                        </a-form-item>
                ...
                <div style="flex: 1;overflow: auto;">
                    <a-card :style="{ width: '100%' }" :title="$t('match.update.5ukj6414ts40')">
                        <a-card style="margin-bottom: 15px;" v-for="(item, index) in form.data.prize">
                            <a-form :model="form.data" layout="vertical" :style="{ maxWidth: '100%', margin: 'auto' }">
                                <a-form-item :label="$t('match.update.5ukj6414tu80')">
                                    <a-input-number hide-button v-model="item.from" :placeholder="$t('match.update.5ukj6414twg0')" />
                                </a-form-item>
                                <a-form-item :label="$t('match.update.5ukj6414tzw0')">
                                    <a-input-number hide-button v-model="item.to" :placeholder="$t('match.update.5ukj6414u1s0')" />
                                </a-form-item>
                                <a-form-item :label="$t('match.update.5ukj6414u580')">
                                    <a-select v-model="item.type" :placeholder="$t('match.update.5ukj6414t000')">
                                        <a-option v-for="item in useEnums('cms.operate.symbol.match.rewardType')"
                                            :value="item.value">{{
                                                item.trans[local.lang] }}</a-option>
                                    </a-select>
                                </a-form-item>
                                <a-form-item :label="$t('match.update.5ukj6414u7c0')">
                                    <a-input v-model="item.text" :placeholder="$t('match.update.5ukj6414u940')" />
                                </a-form-item>
                                <a-form-item :label="$t('match.update.5ukj6414uaw0')">
                                    <div style="width: 100%">
                                        ...
                                        <p style="margin-top:15px">{{$t('match.update.5ukj6414ucw0')}}</p>
                                    </div>
                                </a-form-item>
                                <a-form-item>
                                    <a-button @click="deleteBtn(index)">
                                        ...
                                        {{$t('match.update.5ukj6414ueo0')}}
                                    </a-button>
                                </a-form-item>
                            </a-form>
                        </a-card>
                        <a-button type="primary" @click="form.data.prize.push({
                            from: '', to: '', type: '', text: '', images: []
                        })">
                            <template #icon>
                                <icon-plus />
                            </template>
                            {{$t('match.update.5ukj6414ugg0')}}
                        </a-button>
                    </a-card>
                </div>
            </div>
...
const validatorTime = (value: any, cb: any) => {
    if (form.data.start_time > value) {
        cb(t('match.update.5ukj6414uk00'))
    } else if (value <= new Date().getTime()) {
        cb(t('match.update.5ukj6414ulo0'))
    } else {
        return true
    }
}
const form: any = reactive({
    ...
    rules: {
        'title.zh-CN': [{ required: true, message: t('match.update.5ukj6414si80') }],
        'title.en': [{ required: true, message: t('match.update.5ukj6414sow0') }],
        'title.tc': [{ required: true, message: t('match.update.5ukj6414sts0') }],
        type: [{ required: true, message: t('match.update.5ukj6414unc0') }],
        period: [{ required: true, message: t('match.update.5ukj6414uow0') }],
        is_cycle: [{ required: true, message: t('match.update.5ukj6414uqo0') }],
        status: [{ required: true, message: t('match.update.5ukj6414usc0') }],
        banner: [{ required: true, message: t('match.update.5ukj6414uvk0') }],
        background: [{ required: true, message: t('match.update.5ukj6414uxc0') }],
        'start_time': [{ required: true, message: t('match.update.5ukj6414uz40') }],
        'end_time': [{ required: true, message: t('match.update.5ukj6414v0o0') }, { validator: validatorTime }],
    }
})
...
const submit = async () => {
    ...
    form.loading = true
    const { code, msg } = await apiCms.cmsMatchUpdate({
...
const getData = async () => {
    ...
    form.data.banner = [{
        uid: '1',
        name: form.data.banner,
        url: form.data.banner,
        response: {
            url: form.data.banner,
        }
    }]
    ...
    form.data.prize = JSON.parse(form.data.prize)
    if (form.data.prize?.length > 0) {
        form.data.prize.forEach((item: any) => {
            if (item.text.includes("\\u")) {
                item.text = unescapeText(item.text)
            }
            if (item.from) {
                item.from = Number(item.from)
            }
            if (item.to) {
                item.to = Number(item.to)
            }
            if (item.type) {
        ...
        const isSize = new Promise(function (resolve, reject) {
            ...
            () => {
                if (['image/jpeg', 'image/png', 'image/gif'].includes(file.type)) {
                    ...
                    Message.info(t('match.update.5ukj6414v2c0'))
                }
                return file;
            },
            () => {
                Message.error(t('match.update.5ukj6414v3o0'));
