                    ...
                    <a-col :xs="24">
                        <a-form-item field="payment_type_list" :label="$t('system.update.5ukkfp5va340')"
                            ...
                            <a-select allow-search allow-clear multiple v-model="form.data.payment_type_list"
                                ...
                                <a-option
                                    v-for="item in useEnums('cms.bankCard.system.payment_type')?.filter((item: any) => item.value < 6)"
                                    :value="item.value">
                                    {{ item.trans[local.lang] }}
                                </a-option>
                            </a-select>
                    ...
                    <template v-if="form.data.payment_type_list.includes(3)">
                        ...
                        <a-col :xs="24" :sm="12" :md="8">
                            <a-form-item field="fps_code" label="FPS ID"
                                :rules="[{ required: true, message: '请输入FPS ID' }]">
                                <a-input v-model="form.data.fps_code" placeholder="请输入FPS ID" />
                            </a-form-item>
                        </a-col>
                    </template>
                </a-row>
...
const submit = async () => {
    ...
    if (validate) return;
    form.loading = true
    const { code, msg } = await apiCms.cmsOrderSystemBankCardUpdate({
...
const getData = async () => {
    form.loading = true
    const { code, data } = await apiCms.cmsOrderSystemBankCardInfo({
        cardId: route.params?.id
    })
    form.loading = false
    if (code != 1) return;
