<template>
    <a-form ref="formRef" :model="form.data" :rules="(form.rules as any)" layout="vertical"
        ...
        <a-card :title="$t('task.register.5umxngka3mo0')">
            <a-row :gutter="16">
                ...
                <a-col :xs="24" :sm="12" :md="6">
                    <a-form-item :label="$t('task.register.5umxngka4d40')">
                        {{ props.detail?.from_num || 0 }}{{$t('task.register.5umxngka4gk0')}}{{props.detail?.type == 1? $t('task.register.5umxngka4j40') : $t('task.register.5umxngka4m00') }}{{ props.detail?.to_num }}{{$t('task.register.5umxngka4gk0')}}
                    </a-form-item>
                </a-col>
            </a-row>
        </a-card>
...
const submit = async () => {
    ...
    if (validate) return;
    form.loading = true
    const { code } = await apiTrs.trsSymbolSplitRecord({
        id:props.detail?.id
    })
    form.loading = false
    if (code != 1) return;
