<template>
    <a-form ref="formRef" :model="form.data" :rules="(form.rules as any)" layout="vertical"
        ...
        <a-card :title="$t('task.confirm.5umx9mggn500')">
            <a-row :gutter="16">
                ...
                <a-col :xs="24" :sm="12" :md="6">
                    <a-form-item :label="$t('task.confirm.5umx9mggnrk0')">
                        {{ props.detail?.currency }}
                    </a-form-item>
                </a-col>
                <a-col :xs="24" :sm="12" :md="6">
                    <a-form-item :label="$t('task.confirm.5umx9mggntc0')">
                        {{$t('task.confirm.5un82m353q80')}}{{ props.detail?.volume || 0 }}{{$t('task.confirm.5un82m356c00')}}{{ props.detail?.amount }}
                    </a-form-item>
                </a-col>
            </a-row>
        </a-card>
    ...
    try {
        ...
        reader.onload = async (e: any) => {
            ...
            let arr = jsonData.map((item: any) => {
                ...
                return {
                    position_item_id: item[14],
                    volume: Number(item[9]),
                    amount: Number(item[10]),
                    currency: item[5],
                    register_num: Number(item[7]),
                    dividend_amount: Number(item[11]),
                    exchange_dividend_amount: Number(item[13]),
                }
            })?.filter((_:any,index:any)=>index>0)
            const { code,msg } = await apiTrs.trsSymbolDividendRecordCreate({
