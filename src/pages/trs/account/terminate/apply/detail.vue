            ...
            <a-card :loading="loading">
                <template #title>
                    <div style="display: flex;justify-content: space-between;">
                        ...
                        <a-space :size="18" v-permission="['trsAccountTerminateApplyDetail']">
...
const submit = async () => {
    ...
    audit.loading = true
    const { code, msg } = await apiTrs.accountCancelApplyCheck({
...
const getData = async () => {
    loading.value = true
    const { code, data } = await apiTrs.accountCancelApplyInfo({
        apply_id: route.params?.id
    })
    loading.value = false
    if (code != 1) return;
