                    ...
                    <template #columns>
                        ...
                        <a-table-column :title="$t('apply.apply.5um8hcxvdng0')" :width="150">
                            <template #cell="{ record }">
                                ...
                                <div>{{$t('apply.apply.5um8nsanud40')}}：{{ dayjs.unix(record?.trs_account_info?.expire_time).format('YYYY-MM-DD') }}</div>
                            </template>
                        </a-table-column>
                        ...
                        <a-table-column fixed="right" :title="$t('apply.apply.5um8hcxvdr00')" :width="80"
                            v-if="$permission(['trsAccountTerminateApplyDetail'])">
                            <template #cell="{ record }">
                                <a-space>
                                    <a-link
                                        @click="router.push({ name: 'trsAccountTerminateApplyDetail', params: { id: record.id } })">{{$t('apply.apply.5um8hcxvdu40')}}</a-link>
                                </a-space>
                            </template>
                        </a-table-column>
                    </template>
...
const getData = async () => {
    ...
    const { code, data } = await apiTrs.accountCancelApplyList({
