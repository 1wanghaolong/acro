            ...
            <a-card :loading="loading">
                <template #title>
                    <div style="display: flex;justify-content: space-between;">
                        ...
                        <a-space :size="18" v-permission="['trsAccountFinanceApplyAudit']">
                ...
                <a-form ref="formRef" :model="form.data" auto-label-width layout="vertical">
                    <a-row :gutter="16">
                        ...
                        <a-col :xs="24" :sm="12" :md="8" :xl="6">
                            <a-form-item :label="$t('apply.detail.5um8lff2h1g0')">
                                <div>{{ form.data?.before_finance }}</div>
                            </a-form-item>
                        </a-col>
                        <a-col :xs="24" :sm="12" :md="8" :xl="6">
                            <a-form-item :label="$t('apply.detail.5um8lff2h340')">
                                <div>{{Number(form.data?.after_finance)-Number(form.data?.before_finance) > 0 ? '+' : ''}}{{ Number(form.data?.after_finance)-Number(form.data?.before_finance) }}</div>
                            </a-form-item>
                        </a-col>
                        <a-col :xs="24" :sm="12" :md="8" :xl="6">
                            <a-form-item :label="$t('apply.detail.5um8lff2h5s0')">
                                <div>{{ form.data?.after_finance }}</div>
                            </a-form-item>
                        </a-col>
                        ...
                        <a-col :xs="24" :sm="12" :md="8" :xl="6">
                            <a-form-item :label="$t('apply.detail.5um8i5iqt3w0')">
                                <a-tag size="small" :color="form.data?.status == 2?'#00b42a':form.data?.status == 1?'#ff7d00':'#f53f3f'">
                                    {{ useEnumsFormat('trs.account.finance.apply.status',form.data?.status) }}
                                </a-tag>
                            </a-form-item>
                        </a-col>
        ...
        <a-modal v-model:visible="audit.show" :title="audit.data.status == 2? $t('apply.detail.5um8i5iqqcc0') : $t('apply.detail.5um8i5iqqjk0') " @cancel="audit.show = false" @before-ok="submit">
            <a-form ref="auditFormRef" :model="audit.data" auto-label-width>
                ...
                <template v-if="audit.data.status == 2">
                    TRS{{$t('apply.detail.5um8lsb6x5w0')}}
                </template>
                <!-- 驳回 -->
                <template v-else>
...
const submit = async () => {
    ...
    audit.loading = true
    const { code, msg } = await apiTrs.financeApplyCheck({
...
const getData = async () => {
    loading.value = true
    const { code, data } = await apiTrs.financeApplyInfo({
        apply_id: route.params?.id
    })
    loading.value = false
    if (code != 1) return;
