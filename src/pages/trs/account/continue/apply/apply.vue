                        ...
                        <a-col :xs="24" :sm="12" :md="8" :xl="6">
                            <a-form-item field="status" :label="$t('apply.apply.5um8hcxvd5s0')">
                                <a-select allow-clear v-model="searchInfo.data.status" :placeholder="$t('apply.apply.5um8hcxvd1k0')">
                                    <a-option v-for="item in useEnums('trs.account.terminate.apply.status')"
                                        :value="item.value">{{
                                            item.trans[local.lang] }}</a-option>
                                </a-select>
                            </a-form-item>
                ...
                <a-table :bordered="false" column-resizable :pagination="false" :loading="tableData.loading"
                    ...
                    <template #columns>
                        ...
                        <a-table-column :title="$t('apply.apply.5um8hcxvbrg0')" data-index="asset_account_info.account" :width="120" :ellipsis="true"
                            :tooltip="true"></a-table-column>
                        <a-table-column :title="$t('apply.apply.5um8y34ambs0')" :width="120">
                            <template #cell="{ record }">
                                <div>{{ (record.update_time_limit).toFixed(2) }}{{$t('apply.apply.5unxv4cv0h80')}}</div>
                            </template>
                        </a-table-column>
                        ...
                        <a-table-column :title="$t('apply.apply.5um8hcxvdng0')" :width="150">
                            <template #cell="{ record }">
                                <div>{{$t('apply.apply.5um8nsantdw0')}}：{{ dayjs.unix(record?.trs_account_info?.open_time).format('YYYY-MM-DD') }}</div>
                                <div>{{$t('apply.apply.5um8nsanud40')}}：{{ dayjs.unix(record?.after_expire_time).format('YYYY-MM-DD') }}</div>
                            </template>
                        </a-table-column>
                        <a-table-column :title="$t('apply.apply.5um8hcxvd5s0')" :width="local.lang == 'en' ? 100:100">
                            <template #cell="{ record }">
                                <a-tag size="small"
                                    ...
                                    {{ useEnumsFormat('trs.account.terminate.apply.status', record.status) }}
                                </a-tag>
                            </template>
                        </a-table-column>
                        ...
                        <a-table-column fixed="right" :title="$t('apply.apply.5um8hcxvdr00')" :width="80"
                            v-if="$permission(['trsAccountContinueApplyDetail'])">
                            <template #cell="{ record }">
                                <a-space>
                                    <a-link
                                        @click="router.push({ name: 'trsAccountContinueApplyDetail', params: { id: record.id } })">{{$t('apply.apply.5um8hcxvdu40')}}</a-link>
                                </a-space>
                            </template>
                        </a-table-column>
                    </template>
...
const getData = async () => {
    ...
    const { code, data } = await apiTrs.accountTimeApplyList({
