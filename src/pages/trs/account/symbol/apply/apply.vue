            ...
            <div class="searchBox" :style="{ 'grid-template-rows': !searchInfo.show ? '0fr' : '1fr' }">
                <a-form auto-label-width layout="vertical" :model="searchInfo.data" ref="searchFormRef">
                    <a-row :gutter="16">
                        ...
                        <a-col :xs="24" :sm="12" :md="8" :xl="6">
                            <a-form-item field="symbol" :label="$t('apply.apply.5um8mgtd7040')">
                                <a-input v-model="searchInfo.data.symbol" :placeholder="$t('apply.apply.5um8hcxvcss0')" />
                            </a-form-item>
                        </a-col>
                        <a-col :xs="24" :sm="12" :md="8" :xl="6">
                        ...
                        <a-col :xs="24" :sm="12" :md="8" :xl="6">
                            <a-form-item field="status" :label="$t('apply.apply.5um8hcxvd5s0')">
                                <a-select allow-clear v-model="searchInfo.data.status" :placeholder="$t('apply.apply.5um8hcxvd1k0')">
                                    <a-option v-for="item in useEnums('trs.account.symbol.apply.status')"
                                        :value="item.value">{{
                                            item.trans[local.lang] }}</a-option>
                                </a-select>
                            </a-form-item>
            ...
            <div class="tableBox">
                <a-table :bordered="false" column-resizable :pagination="false" :loading="tableData.loading"
                    ...
                    <template #columns>
                        ...
                        <a-table-column :title="$t('apply.apply.5um8mgtd7g00')" :width="150">
                        ...
                        <a-table-column :title="$t('apply.apply.5um8hcxvbrg0')" data-index="asset_account_info.account" :width="120" :ellipsis="true"
                            :tooltip="true"></a-table-column>
                        <a-table-column :title="$t('apply.apply.5um8hcxvcxs0')" data-index="trs_account_info.currency" :width="80">
                            <template #cell="{ record }">
                                <a-tag>{{ record?.trs_account_info?.currency || $t('apply.apply.5um8l85reqw0') }}</a-tag>
                            </template>
                        </a-table-column>
                        <a-table-column :title="$t('apply.apply.5um8mgtd7kc0')" :width="160">
                            <template #cell="{ record }">
                                <div>{{ record.investment_amount }}({{ Number(record.investment_rate) }}%)</div>
                            </template>
                        </a-table-column>
                        <a-table-column :title="$t('apply.apply.5um8mgtd7ms0')" :width="180">
                            <template #cell="{ record }">
                                <div>{{$t('apply.apply.5um8hxisac80')}}：{{ record?.trs_account_info?.total_cash }}</div>
                                <div>{{$t('apply.apply.5un70vpbpq80')}}：{{ record?.trs_account_info?.total_finance }}</div>
                            </template>
                        </a-table-column>
                        <a-table-column :title="$t('apply.apply.5um8hcxvd5s0')" :width="local.lang =='en'?100:100">
                            <template #cell="{ record }">
                                <a-tag size="small"
                                    :color="record.status == 1 ? '#00b42a' : record.status == 0 ? '#ff7d00' : '#f53f3f'">
                                    {{ useEnumsFormat('trs.account.symbol.apply.status', record.status) }}
                                </a-tag>
                            </template>
                        </a-table-column>
                        ...
                        <a-table-column fixed="right" :title="$t('apply.apply.5um8hcxvdr00')" :width="80"
                            v-if="$permission(['trsAccountSymbolApplyDetail'])">
                            <template #cell="{ record }">
                                <a-space>
                                    <a-link
                                        @click="router.push({ name: 'trsAccountSymbolApplyDetail', params: { id: record.id } })">{{$t('apply.apply.5um8hcxvdu40')}}</a-link>
                                </a-space>
                            </template>
                        </a-table-column>
                    </template>
...
const getData = async () => {
    ...
    const { code, data } = await apiTrs.investmentScopeApplyList({
