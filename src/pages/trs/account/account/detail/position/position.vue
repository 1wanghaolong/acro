<template>
    <div class="tabBox">
        <div class="buttonBox">
                <a-space :size="18">
                    <a-input v-model="searchInfo.data.symbol" :placeholder="$t('position.position.5ukft4xh7z00')" />
                    <a-button @click="searchInfo.data.symbol = '',getData()">
                        <template #icon>
                            <icon-refresh />
                        </template>
                ...
                <a-space :size="18" v-permission="['trsOrderCloseAllPosition']">
                    <a-popconfirm position="left" @ok="closeAllPosition"
                        :content="$t('position.position.5um88odnwxo0')">
                        <a-button type="primary">
                            {{$t('position.position.5um88odnxh80')}}
                        </a-button>
                    </a-popconfirm>
                </a-space>
            </div>
        <div class="tableBox">
            <a-table :bordered="false" column-resizable :pagination="false" :loading="tableData.loading"
                ...
                class="table" :expandable="{ title: '', width: 50 }" row-key="id">
                ...
                <template #columns>
                    ...
                    <a-table-column :title="`TRS${ $t('position.position.5um88odnxn80') }`" data-index="trs_account_info.account" :width="120" :ellipsis="true" :tooltip="true"></a-table-column>
                    ...
                    <a-table-column :title="$t('position.position.5um88odnxz40')" :width="160">
                        <template #cell="{ record }">
                            <div>{{ record.position_item_list?.[0]?.counter_channel_account_info?.name }}</div>
                            <div>{{ record.position_item_list?.[0]?.counter_channel_info?.name }} {{ useEnumsFormat('market.order.counter_channel_scene',record.position_item_list?.[0]?.counter_channel_scene) }}</div>
                        </template>
                    </a-table-column>
                    ...
                    <a-table-column :title="$t('position.position.5um88odnyew0')" data-index="profit_rate" :width="90">
                        <template #cell="{ record }">
                            <div>{{ (record.profit_rate*100).toFixed(2) }}%</div>
                        </template>
                    </a-table-column>
                    <a-table-column fixed="right" :title="$t('position.position.5um88odnyig0')" :width="local.lang == 'en' ? 140 :80" v-if="$permission(['trsOrderClosePosition'])">
                        <template #cell="{ record }">
                            <a-space>
                        ...
                        <template #columns>
                            ...
                            <a-table-column :title="$t('position.position.5um88odnxz40')">
                                <template #cell="{ record }">
                                    ...
                                    <div>{{ record?.counter_channel_info?.name }} {{ useEnumsFormat('market.order.counter_channel_scene',record?.counter_channel_scene) }}</div>
                                </template>
                            </a-table-column>
                            ...
                            <a-table-column fixed="right" :title="$t('position.position.5um88odnyig0')" :width="80" v-if="$permission(['trsOrderClosePosition'])">
                                <template #cell="{ record }">
                                    <a-space>
...
const closeAllPosition = async () => {
    const { code, msg } = await apiTrs.orderCloseAllPosition({
...
const getData = async () => {
    ...
    const { code, data } = await apiTrs.positionList(useFilter({
        ...searchInfo.data,
        trs_account_id: route.params?.id
    }))
