<template>
    <a-tabs v-model:active-key="active" @change="getData" justify>
        <template #extra>
            <a-button type="primary" v-permission="['trsAccountDetailTransferCreate']" @click="form.show = true">
                <template #icon>
                    <icon-plus />
                </template>
                {{$t('transfer.transfer.5um8ckws10w0')}}
            </a-button>
        </template>
                ...
                <div class="tableBox">
                    <a-table :bordered="false" column-resizable :pagination="false" :loading="tableData.loading"
                        ...
                        <template #columns>
                            ...
                            <a-table-column :title="$t('transfer.transfer.5um3uxit5r00')" :width="120" :ellipsis="true" :tooltip="true">
                            ...
                            <a-table-column :title="`TRS${ $t('transfer.transfer.5um3vawfrec0') }`" data-index="trs_account_info.account"
                                :width="120" :ellipsis="true" :tooltip="true"></a-table-column>
                            ...
                            <a-table-column :title="$t('transfer.transfer.5um3uxit6gw0')" :width="80">
                                <template #cell="{ record }">
                                    <a-tag size="small" :color="record.status == 2?'#00b42a':record.status == 1?'#ff7d00':'#f53f3f'">
                            ...
                            <a-table-column :title="$t('transfer.transfer.5um3uxit6x40')" :width="80" v-if="$permission(['otcAccountTransferDetail'])">
                                <template #cell="{ record }">
                                    <a-space>
                                        <a-link
                ...
                <div class="tableBox">
                    <a-table :bordered="false" column-resizable :pagination="false" :loading="tableData.loading"
                        ...
                        <template #columns>
                            ...
                            <a-table-column :title="$t('transfer.transfer.5um3uxit7g80')" :width="local.lang == 'en' ? 140 :80">
                                <template #cell="{ record }">
                                    <a-tag>
        ...
        <a-modal v-model:visible="form.show" :title="$t('transfer.transfer.5um8ckws1l40')" @cancel="form.show = false" @before-ok="submit">
            <a-form ref="formRef" :model="form.data" layout="vertical">
                <a-row :gutter="16">
                    <a-col :xs="24" :sm="12">
                        <a-form-item :label="$t('transfer.transfer.5um3uxit5r00')">
                            <div>{{ accountDetail?.asset_account_info?.account }}</div>
                        </a-form-item>
                    </a-col>
                    <a-col :xs="24" :sm="12">
                        <a-form-item :label="$t('transfer.transfer.5um8ckws1og0')">
                            <div>{{ accountDetail?.usable_power }}</div>
                        </a-form-item>
                    </a-col>
                    <a-col :xs="24" :sm="12">
                        <a-form-item :label="$t('transfer.transfer.5um3uxit62g0')">
                            <div>{{ accountDetail?.currency }}</div>
                        </a-form-item>
                    </a-col>
                    <a-col :xs="24" :sm="12">
                        <a-form-item :label="`TRS${ $t('transfer.transfer.5um3vawfrec0') }`">
                            <div>{{ accountDetail?.account }}</div>
                        </a-form-item>
                    </a-col>
                    <a-col :xs="24" :sm="12">
                        <a-form-item field="amount" :label="$t('transfer.transfer.5um3uxit6cc0')" :rules="[{required:true,message:$t('transfer.transfer.5um8ckws1r40')},{type:'number',min:1,message:$t('transfer.transfer.5unxdv1x7zg0')}]">
                            <a-input-number @change="debounceGetChargeCalculate" v-model="form.data.amount" :placeholder="$t('transfer.transfer.5um8ckws1r40')" />
                        </a-form-item>
                    </a-col>
                    <a-col :xs="24" :sm="12">
                        <a-form-item :label="$t('transfer.transfer.5um8ckws1tk0')">
                            <a-select @change="getChargeCalculate" v-model="form.data.is_auto_calculate_fee" :placeholder="$t('transfer.transfer.5um8ckws1vs0')">
                    ...
                    <a-col :xs="24" :sm="12">
                        <a-form-item :label="$t('transfer.transfer.5um3uxit7bo0')" field="fee" :rules="[{required:true,message:$t('transfer.transfer.5um8ckws2040')}]">
                            <div v-if="form.data.is_auto_calculate_fee == 1">{{ form.data.fee }}</div>
                            <a-input-number  v-else v-model="form.data.fee" :placeholder="$t('transfer.transfer.5um8ckws2040')" />
                        </a-form-item>
                    </a-col>
                    <a-col :xs="24" :sm="12">
                        <a-form-item :label="$t('transfer.transfer.5um8ckws2200')">
                            {{ (form.data.amount - form.data.fee) || 0 }}
                        </a-form-item>
                    </a-col>
                    <a-col :xs="24" :sm="12">
                        <a-form-item :label="$t('transfer.transfer.5um8ckws2400')">
                            {{ form.data.fee || 0 }}
                        </a-form-item>
                    </a-col>
                    <a-col :xs="24" :sm="12">
                        <a-form-item :label="`TRS${ $t('transfer.transfer.5um8d60tl7g0') }`">
                            {{ (form.data.amount - form.data.fee) || 0 }}
                        </a-form-item>
                    </a-col>
                </a-row>
...
const submit = async () => {
    ...
    form.loading = true
    const { code, msg } = await apiTrs.accountChargeTransferCreate({
        data: {
            ...form.data,
